...
<div class="main">
	...
	<span style="padding-top: 20px;padding-left: 24px; font-size:14px;font-weight:700;margin-left:10px;float: left;color: red;background:url(../../../common/images_V6.0/table_icon/icon01.png) no-repeat left 23px;">注意：优先使用产品的快递模板,若产品无设置快递模板,则使用公用快递模板。</span>
	<a style="margin-top: 0px;float: right;" href="express_template_add.php?customer_id=<?php echo passport_encrypt((string)$customer_id);?>&action=add/>"><button class="btn-green mt15 newadd diy_btn" >新建模板</button></a>
	
	<div style="float: right;" class="search-box">
		<input class="search-text" type="text" placeholder="模板名称" value="<?php echo $search_keyword;?>" id="search_keyword">
	...
	<div class="content-box">
		<table>
			...
			<tbody>
				<?php
				    //运费模板信息 
					$query_temid="SELECT id,title,is_default,createtime FROM express_template_t where isvalid=true and customer_id=".$customer_id." and supply_id=-1";
					if($search_keyword){
						$query_temid = $query_temid."  and title like '%".$search_keyword."%'";
					}
				...
				<tr >
					...
					<td style="border-right: none;">
						<a style="margin-right: 8px;" href="express_template_add.php?customer_id=<?php echo passport_encrypt((string)$customer_id);?>&tem_id=<?php echo $tem_id;?>&title=<?php echo $title;?>&op=update">
						...
						<?php if($isused){?>
						<button title="模板已启用" style="background: none;border: none;margin-right: 8px;" class="btn-green diy_btn" onclick="temp_cancel(<?php echo $tem_id;?>)"><img width="20" height="20" src="../../../common/images_V6.0/operating_icon/icon74.png"/></button>					
						<?php }else{?>
						<button title="启用此模板" style="background: none;border: none;margin-right: 8px;" class="btn-white " onclick="temp_check(<?php echo $tem_id;?>)"><img width="20" height="20" src="../../../common/images_V6.0/operating_icon/icon75.png"/></button>
						<?php }?>
					</td>
...
$(function(){
	...
	$("#delAll").click(function() {
		...
		layer.confirm('确定要删除吗？', {
			...
			$.ajax({  
				type : "POST",  
				url : "express_template.class.php?op=deleteall",
...
function changename(tem_id,name){  //修改模板名字
	...
	$.ajax({  
		type : "POST",  
		url : "express_template.class.php",
...
function temp_delete(tem_id){ //删除模板
		layer.confirm('确定要删除吗？', {
			...
			$.ajax({  
				type : "POST",  
				url : "express_template.class.php?op=del",
				data : {"tem_id" : tem_id},
				dataType: "json",		
				success : function(result) {
					if(result.code=="1"){
...
function temp_check(tem_id){ //选择模板
	layer.confirm('只能选择一个模板，确定选择此模板吗？', {
			title: false,
			skin:'red-skin',
			//shift:6,
  			btn: ['确认','取消'] //按钮
			...
			$.ajax({  
				type : "POST",  
				url : "express_template.class.php?op=express_check",	
				data : {"tem_id" : tem_id},
				dataType: "json",		
				success : function(result) {
					if(result.code=="1"){
...
function prePage(){
	pagenum--;
	document.location= "express_template.php?pagenum="+pagenum+"&customer_id=<?php echo passport_encrypt((string)$customer_id); ?>";
}
...
function search(){
	...
	document.location= "express_template.php?pagenum="+pagenum
...
function gopage(v){
	...
	if(a.hasClass('one')){
		...
		document.location= "express_template.php?customer_id=<?php echo passport_encrypt((string)$customer_id) ?>&pagenum="+a.val();
	}
}
function jumppage(){
	...
	if((a<1) || (a==pagenum) || (a>page) || isNaN(a)){
		...
		document.location= "express_template.php?customer_id=<?php echo passport_encrypt((string)$customer_id) ?>&pagenum="+a;
	}
}
